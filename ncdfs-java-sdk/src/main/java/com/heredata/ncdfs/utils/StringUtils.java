...
public class StringUtils {
    ...
    public static byte[] stringToByteArray(String str) {
        byte[] rawData = null;
        try {
            rawData = str.getBytes(DEFAULT_CHARSET_NAME);
     ...
     * @return
     */
    public static String getPointLengthUUID(int length) {
        if (length <= 0) {
            throw new IllegalArgumentException("legnth must greater than 0");
        }

        int count = (length / 36) + 1;
     ...
     * 对指定的字符串进行MD5加密
     */
    public static byte[] encrypByMD5Arr(String originString) {
        try {
            //创建具有MD5算法的信息摘要
