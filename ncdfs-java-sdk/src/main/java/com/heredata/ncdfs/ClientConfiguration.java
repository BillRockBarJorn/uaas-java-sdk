...
import com.heredata.ncdfs.comm.IdleConnectionReaper;
import com.heredata.ncdfs.comm.Protocol;
import com.heredata.ncdfs.comm.RetryStrategy;
import com.heredata.ncdfs.exception.NCDFSClientException;
import com.heredata.ncdfs.internal.NCDFSConstants;
import com.heredata.ncdfs.utils.ResourceManager;
...
/**
 * Client configurations for accessing to HOS services.
 */
public class ClientConfiguration {
    ...
    public static final String DEFAULT_CNAME_EXCLUDE_LIST = "heredata.com";
     ...
     */
    public void setProxyPort(int proxyPort) throws NCDFSClientException {
        if (proxyPort <= 0) {
            throw new NCDFSClientException(
                    ResourceManager.getInstance(NCDFSConstants.RESOURCE_NAME_COMMON).getString("ParameterIsInvalid"),
                    null);
        }
    ...
    /**
     * Gets the HOS's protocol (HTTP or HTTPS).
     */
    ...
    /**
     * Sets the HOS's protocol (HTTP or HTTPS).
     */
     ...
     * HOS's token validation logic depends on the time.
     * It requires that there's no more than 15 min time difference between client and HOS server.
     * This API calculates the difference between local time to epoch time.
     * Later one other APIs use this difference to offset the local time before sending request to HOS.
     *
     * @param epochTicks
