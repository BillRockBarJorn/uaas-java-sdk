...
import com.heredata.s3.S3ErrorCode;
 ...
 */
public class S3ErrorResponseHandler implements ResponseHandler {
    ...
    public void handle(ResponseMessage response) throws ServiceException, ClientException {
        ...
        if (response.getContent() == null && statusCode != null) {
            ...
            if (statusCode == HttpStatus.SC_NOT_FOUND) {
                throw ExceptionFactory.createServiceException(requestId, statusCode + "", S3ErrorCode.NO_SUCH_KEY);
            } else if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
                throw ExceptionFactory.createServiceException(requestId, statusCode + "", S3ErrorCode.NOT_MODIFIED);
            } else if (statusCode == HttpStatus.SC_PRECONDITION_FAILED) {
                throw ExceptionFactory.createServiceException(requestId, statusCode + "", S3ErrorCode.PRECONDITION_FAILED);
            } else if (statusCode == HttpStatus.SC_FORBIDDEN) {
                throw ExceptionFactory.createServiceException(requestId, statusCode + "", S3ErrorCode.ACCESS_FORBIDDEN);
            } else {
