...
import static com.heredata.s3.comm.S3Headers.HOS_PREFIX;
import static com.heredata.s3.comm.S3Headers.X_HOS_DATE;
...
public class S3Signer implements RequestSigner {
    ...
    private String date;
    protected final S3SignerParams signerParams;

    public S3Signer(S3SignerParams signerParams) {
    ...
    protected void addSecurityTokenHeaderIfNeeded(RequestMessage request) {
        ...
        if (cred.useAccount() && !request.isUseUrlSignature()) {
            request.addHeader(S3Headers.HOS_SECURITY_TOKEN, cred.getAccount());
        }
    }
...
//        }
    ...
    private void addDateHeaderIfNeeded(RequestMessage request) {
        ...
        request.getHeaders().put(S3Headers.HOS_DATE, date);
    }
    ...
    protected void addAuthorizationHeader(RequestMessage request) {
        ...
        String authorization = String.format("AWS %s:%s", signerParams.getCredentials().getAccessKey(), signature);
        request.addHeader(S3Headers.AUTHORIZATION, authorization);
