...
import com.heredata.s3.internal.Mimetypes;
...
public class S3UploadOperation {
    ...
    public S3UploadOperation(S3MultipartOperation multipartOperation) {
    ...
    private CompleteMultipartUploadResult uploadFileWithCheckpoint(UploadObjectRequest uploadObjectRequest) throws Throwable {
        ...
        if (multipartOperation.getInnerClient().getClientConfiguration().isCrcCheckEnabled()) {
            ...
            try {
                S3Utils.checkChecksum(clientCRC, multipartUploadResult.getServerCRC(), multipartUploadResult.getRequestId());
            } catch (Exception e) {
    ...
    class Task implements Callable<PartResult> {
        ...
        public Task(int id, String name, UploadCheckPoint uploadCheckPoint, int partIndex,
                    UploadObjectRequest uploadObjectRequest, S3MultipartOperation multipartOperation,
        ...
        public PartResult call() throws Exception {
            ...
            try {
                ...
                if (multipartOperation.getInnerClient().getClientConfiguration().isCrcCheckEnabled()) {
                    S3Utils.checkChecksum(uploadPartResult.getClientCRC(), uploadPartResult.getServerCRC(), uploadPartResult.getRequestId());
    ...
    private CompleteMultipartUploadResult complete(UploadCheckPoint uploadCheckPoint,
        ...
        ObjectMetadata metadata = uploadObjectRequest.getObjectMetadata();
        if (metadata != null) {
            String acl = (String) metadata.getRawMetadata().get(S3Headers.HOS_OBJECT_ACL);
...
//        completeUploadRequest.setCallback(uploadFileRequest.getCallback());
    ...
    protected S3MultipartOperation multipartOperation;
}
