...
import com.heredata.s3.model.bucket.Bucket;
import com.heredata.s3.model.bucket.BucketList;
import com.heredata.s3.model.bucket.BucketQuotaResult;
import com.heredata.s3.model.bucket.BucketVersioningConfiguration;
import com.heredata.s3.operation.*;
...
public class S3Client implements S3 {
    ...
    private S3BucketOperation bucketOperation;
    private S3ObjectOperation objectOperation;
    private S3MultipartOperation multipartOperation;
    private S3UploadOperation uploadOperation;
    private S3DownloadOperation downloadOperation;
    private S3AccountOperation accountOperation;

    /**Gets the inner multipartOperation, used for subclass to do implement opreation.*/
    public S3MultipartOperation getMultipartOperation() {
        return multipartOperation;
    }

    /**Gets the inner objectOperation, used for subclass to do implement opreation.*/
    public S3ObjectOperation getObjectOperation() {
        return objectOperation;
    }

    /**Sets the inner downloadOperation.*/
    public void setDownloadOperation(S3DownloadOperation downloadOperation) {
        this.downloadOperation = downloadOperation;
    }

    /**Sets the inner uploadOperation.*/
    public void setUploadOperation(S3UploadOperation uploadOperation) {
        this.uploadOperation = uploadOperation;
    }

    /**
     * Uses the specified {@link CredentialsProvider}, client configuration and
     * HOS endpoint to create a new {@link S3Client} instance.
     *
     * @param endpoint
     ...
     */
    public S3Client(String endpoint, CredentialsProvider credsProvider, ClientConfiguration config) {
    ...
    public S3Client(String endpoint, CredentialsProvider credsProvider, ClientConfiguration config, String bucket) {
    ...
    public synchronized void setEndpoint(String endpoint) {
        ...
        S3Utils.ensureEndpointValid(uri.getHost());
    ...
    private URI toURI(String endpoint) throws IllegalArgumentException {
        return S3Utils.toEndpointURI(endpoint, this.serviceClient.getClientConfiguration().getProtocol().toString());
    }

    private void initOperations() {
        this.bucketOperation = new S3BucketOperation(this.serviceClient, this.credsProvider);
        this.objectOperation = new S3ObjectOperation(this.serviceClient, this.credsProvider);
        this.multipartOperation = new S3MultipartOperation(this.serviceClient, this.credsProvider);
        this.uploadOperation = new S3UploadOperation(this.multipartOperation);
        this.downloadOperation = new S3DownloadOperation(objectOperation);
        this.accountOperation = new S3AccountOperation(this.serviceClient, this.credsProvider);
    }
    ...
    /**
     * Deprecated. Please use {@link S3Client#doesBucketExist(String)} instead.
     */
    @Deprecated
